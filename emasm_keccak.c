 ...
 * Modified for purpose of EMASM by Mikhail Baynov m.baynov@gmail.com
 * ---------------------------------------------------------------------- */
...
char* keccak_hash(const char* in) {
  char *out = NEW_LINE;
  char *out_p = out;
  uint8_t *buf = calloc(200,1);
  sha3_context c;
	sha3_Init256(&c);
	sha3_Update(&c, in, strlen(in));
	uint8_t *hash;
...
int keccak_test() {
	char *str = keccak_hash("identity3()");
	char *str2 = keccak_hash("identity2()");
	if (strcmp(str, "f00032def569b31e64ba9c6846671eab2395fea87915ab639291340bf42a47c3")) {
		printf("f00032def569b31e64ba9c6846671eab2395fea87915ab639291340bf42a47c3\n");
		printf("%s\n", str);

		printf("956c26ebd0e803bdda8acb2fadb6e8028958b76a3d0feccd74003a85f00de905\n");
		printf("%s\n", str2);

		printf("KECCAK HASH TEST FAILED\n");
		return 0;
	} else return 1;
}
